...
- **FileOps: Claude Code-style read/edit/write digest workflow** ([#146](https://github.com/Oaklight/toolregistry-hub/pull/146)) — replaces the deprecated `read_file` / `write_file` / `append_file` surface with a stateless, digest-based API:
    - `read(path)` → `{content, digest, is_symlink, real_path}` — reads file bytes, returns SHA-256 digest as a version token. Symlinks are allowed for reading; `is_symlink` and `real_path` are reported so callers can pass the real path to write operations.
    - `edit(path, old_string, new_string, digest, replace_all=False, start_line=None)` → `{diff, digest}` — exact string replacement guarded by digest; returns an updated digest so edits can chain without re-reading.
    - `write(path, content, digest=None, mode="overwrite"|"append")` → `{digest}` — atomic overwrite or append; new files need no digest, existing files require a matching digest to prevent silent overwrites. `append` mode decodes the existing file with its original encoding/BOM, appends as a string, then re-encodes, preserving UTF-16 and BOM-prefixed files correctly.
    - `edit` and `write` reject symlink paths (including `.tmp` intermediates). External modifications invalidate the digest and force a fresh `read`. 30 tests; no server-side state.
