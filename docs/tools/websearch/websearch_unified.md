...
- `engine` (str): Provider to use. `"auto"` (default) tries configured engines in priority order. `"parallel"` queries multiple engines concurrently and deduplicates with BM25 re-ranking. Specific values: `"brave"`, `"tavily"`, `"searxng"`, `"brightdata"`, `"scrapeless"`, `"serper"`, `"reddit"`, `"github"`
