...
- **Soup 跳过优化**：当 readability 产出高置信度结果（score ≥ 100、内容 ≥ 2,000 字符）时，完全跳过 soup 提取步骤，节省 30–185 ms
...
- **Soup 跳过机制**：当 readability 评分较高（≥ 100）且内容量充足（≥ 2,000 字符）时，会跳过 soup 提取以节省时间。极少数情况下可能遗漏 soup 能捕获的内容。跳过的调用会以 DEBUG 级别记录，便于监控
