"""Unit tests for UnifiedFileTools."""
...
from toolregistry_hub.unified_file_tools import UnifiedFileTools


class TestUnifiedFileTools:
    ...
    def test_read_file_single(self):
        result = UnifiedFileTools.read_file([self.test_file])
        assert len(result) == 1
        item = result[0]
    ...
    def test_read_file_multiple(self):
        ...
        with open(file2, "w") as f:
            f.write("Second file")
        results = UnifiedFileTools.read_file([self.test_file, file2])
    ...
    def test_read_file_truncated_by_words(self):
        ...
        result = UnifiedFileTools.read_file([huge_file])
        assert len(result) == 1
        item = result[0]
        assert item["status"] == "success"
    ...
    def test_write_to_file(self):
        ...
        result = UnifiedFileTools.write_to_file(new_file, new_content)
        assert result["success"]
        assert os.path.exists(new_file)
        read_result = UnifiedFileTools.read_file([new_file])
    ...
    def test_insert_content_append(self):
        result = UnifiedFileTools.insert_content(self.test_file, 0, "Appended\n")
        assert result["success"]
        read_result = UnifiedFileTools.read_file([self.test_file])
        content_lines = read_result[0]["content"].splitlines()
    ...
    def test_insert_content_line2(self):
        result = UnifiedFileTools.insert_content(self.test_file, 2, "Inserted\n")
        assert result["success"]
        read_result = UnifiedFileTools.read_file([self.test_file])
        content_lines = read_result[0]["content"].splitlines()
    ...
    def test_insert_content_invalid_line(self):
        result = UnifiedFileTools.insert_content(self.test_file, 10, "Invalid")
        assert not result["success"]

    def test_apply_diff_unified(self):
        UnifiedFileTools.write_to_file(self.test_file, "line1\nline2\nline3\n")
        diff = """--- a/test.txt
+++ b/test.txt
@@ -1,3 +1,3 @@
 line1
...
+new line2
 line3"""
        result = UnifiedFileTools.apply_diff(self.test_file, diff, "unified")
        assert result["success"]
        read_result = UnifiedFileTools.read_file([self.test_file])
    ...
    def test_apply_diff_git(self):
        UnifiedFileTools.write_to_file(self.test_file, "old\nline3\n")
        diff = """<<<<<<< SEARCH
old
=======
new
>>>>>>> REPLACE"""
        result = UnifiedFileTools.apply_diff(self.test_file, diff, "conflict")
        assert result["success"]
        read_result = UnifiedFileTools.read_file([self.test_file])
        content = read_result[0]["content"]
    ...
    def test_search_and_replace_literal(self):
        result = UnifiedFileTools.search_and_replace(
    ...
    def test_search_files(self):
        ...
        with open(file2, "w") as f:
            f.write("test match")
        results = UnifiedFileTools.search_files(self.temp_dir, "test")
    ...
    def test_list_files(self):
        ...
        os.mkdir(subdir)
        results = UnifiedFileTools.list_files(self.temp_dir, recursive=False)
        assert "test.txt" in results
        assert "sub" in results

        rec = UnifiedFileTools.list_files(self.temp_dir, recursive=True)
        assert len(rec) > 0
